...
def transform(): #Transformacija na Rechnik
    for person in UserRatings:
        for film in UserRatings[person]:
            if film not in MovieRatings:
                MovieRatings[film]={}
            MovieRatings[film][person]=UserRatings[person][film]
...
if __name__ == "__main__":
    ...
    for movie1 in movies_watched:
        print("|",movie1,"\t","|")
        print("------------------------------")
        for movie2 in sorted(MovieRatings.keys()):
            if movie1==movie2:
                continue
            if movie1 in movies_watched:
                result=cos_similarity(MovieRatings, movie1, movie2)
                if result>=threshold:
                    print(movie2 +"\t",cos_similarity(MovieRatings, movie1, movie2))
                #print(cos_similarity(MovieRatings, movie1, movie2), sim_pearson(MovieRatings, movie1, movie2),
                      #sim_distance(MovieRatings, movie1, movie2))
        print("------------------------------")
